...
repo: vortech
package_name: "SystemAgent.Contract"
package_path: "system-agent/contract/cs"
language: csharp
manifest: SystemAgent.Contract.csproj
...
# SystemAgent.Contract

## 1. Identidad
- **Nombre:** SystemAgent.Contract
- **Path:** `system-agent/contract/cs`
- **Lenguaje:** csharp
...
### 2.2 Inferido
Package clasificado por manifest `SystemAgent.Contract.csproj` en el monorepo Vortech.

**Evidencia:**
- `system-agent/contract/cs/Actions/ActionRequest.cs:1` — primer fuente local bajo el path (heurística Fase 1).
...
- Inspeccionar subcarpetas `src/`, `lib/`, `tests/` bajo `system-agent/contract/cs`.
