...
repo: vortech
package_name: "v-alloy"
package_path: "rust-workspace/alloy"
language: rust
manifest: Cargo.toml
...
### 2.2 Inferido
Package clasificado por manifest `Cargo.toml` en el monorepo Vortech.

**Evidencia:**
- `rust-workspace/alloy: (sin archivo fuente típico)` — primer fuente local bajo el path (heurística Fase 1).
...
- Inspeccionar subcarpetas `src/`, `lib/`, `tests/` bajo `rust-workspace/alloy`.
